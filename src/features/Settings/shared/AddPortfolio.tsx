...
const AddPortfolio = ({
  ...
  const { setPortfolioLoader, portfolioLoader } =
  ...
  return (
    ...
    >
      <form onSubmit={handleAddPortfolio}>
        {portToUpdate?.id &&
          (portToUpdate?.attributes?.Gallery?.data || imagesrc) && (
            <div>
              <Image
                width={500}
                height={500}
                alt="the profile picture"
                className="max-h-[350px] w-full object-cover rounded-[6px]"
                src={
                  imagesrc
                    ? imagesrc
                    : portToUpdate?.attributes?.Gallery?.data
                    ? portToUpdate?.attributes?.Gallery?.data[0]?.attributes
                        ?.url
                    : ""
                }
              />
        ...
        <div className="mt-4 w-full  flex flex-col space-y-1">
          <div className="grid w-full max-w-sm items-center gap-1.5">
            ...
            <label className='flex items-center gap-[10px] h-10 w-full rounded-md border border-input bg-background px-3 py-1 text-sm ring-offset-background file:border- file:bg-transparent file:text-sm file:font-medium placeholder-gray-500 placeholder-opacity-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2z disabled:cursor-not-allowed disabled:opacity-50'>
              <input
                type="file"
                name=""
                id=""
        ...
        <div className="mt-4 w-full  flex flex-col space-y-1">
          <h1 className="text-[#808191] text-[14px] leading-[22px]">
            Name (max : 100 characters)<span className="text-red-500">*</span>
          </h1>
          <Input
            name="name"
            value={form.name}
            ...
            required
            className="bg-[#FCFCFC] border border-[#E4E4E4] px-[10px] rounded-[6px] py-[10px] placeholder-gray-500 placeholder-opacity-50 "
        ...
        <div className="mt-4 w-full flex flex-col space-y-1">
          <h1 className="text-[#808191] text-[14px] leading-[22px]">
            Price (max : 40 characters)<span className="text-red-500">*</span>
          </h1>
        ...
        <div className="mt-4 flex flex-col space-y-1 my-2">
          <div className="flex items-center justify-between">
            ...
            <IconShowcaseBox
              ...
              color="#3EB87F"
              width="w-fit"
              textCN="text-[12px] font-semibold leading-[26px] text-white"
              px="10px"
              py="2px"
              rounded={"4px"}
              noBorder
        ...
        <IconShowcaseButton
          disabled={loader ? loader : deleteLoader ? deleteLoader : false}
          loading={loader}
          text={`${portToUpdate?.id ? "Update" : "Add"} Portfolio`}
          color="#3EB87F"
          textCenter
          width="w-full"
      ...
      {portToUpdate?.id && (
        <IconShowcaseButton
          disabled={loader ? loader : deleteLoader ? deleteLoader : false}
          onClick={() => deletePort && deletePort(portToUpdate?.id)}
      ...
      >
        ...
        {response?.trim() !== "" && (
          <div className={"flex justify-start my-5"}>
            <div
              className={`py-4 px-4 text-white ${"bg-slate-800 max-w-[100%]"} ${"rounded-b-[6px] rounded-tr-[6px]"}`}
            >
              <div>
                <h1> {response}</h1>
              </div>
