            ...
            <div className='max-md:flex max-md:flex-wrap max-md:justify-center md:grid md:grid-cols-2 xl:grid-cols-3 md:gap-4' >

                {
                    servicesData?.map((service: Service) => (
                        // <AgentEvent key={service?.id} title={service?.attributes?.name} description={service.attributes.description} location={service.attributes.location} price={service.attributes.price} noArrow />
                        <ServicesCard banner={service?.attributes?.Gallery} key={service?.id} title={service?.attributes?.name} description={service.attributes.description} location={service.attributes.location} price={service.attributes.price} noArrow />

                        // <FeedPropertyCard key={service?.id} title={service?.attributes?.name} description={service.attributes.description} location={service.attributes.location} price={service.attributes.price} featuredClassName />
                    ))
                }
