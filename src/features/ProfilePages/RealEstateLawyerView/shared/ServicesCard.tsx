...
const ServicesCard = ({ height, noArrow, title, description, location, price, featuredClassName, banner }: Props) => {
    return (
        <div className={`w-5/6 ${featuredClassName ? "md:w-[338px]" : "md:w-[290px]"} h-full cursor-pointer md:mx-2 my-2 flex-shrink-0 border border-black rounded-xl`}>
            {/* the property Image */}
            <Image height={1000} width={1000} className='h-[240px] rounded-t-xl object-cover' alt='property Image' src={banner?.data?.length > 0 ? banner?.data[0]?.attributes?.url : "/Broker/Services.png"} />
            {/* The lower div */}
            <div className='py-5 px-5 space-y-3'>
                ...
                {/* the Title */}
                <h1 className='text-[16px] font-normal text-[#34495D]'>{title ?? "Property Title"}</h1>
                {/* the description */}
                <h1 className='text-[12px] font-normal text-black'>{description ?? "Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Aenean commodo ligula eget dolor. Aenean massa."}</h1>
                <div className='flex space-x-2 items-center my-4'>
                    ...
                    <h1 className='text-[12px] font-normal text-[#34495D]'>{location ?? "North Carolina, USA"}</h1>
                </div>
