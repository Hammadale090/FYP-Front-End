...
const AgentReview = ({ id }: Props) => {
    const { jwt, profileId } = useContext(AuthContext)
    const { reviewsLoader, setReviewsLoader } = useContext(ProfSettingsContext)
        ...
        try {
            const responseData = await fetcher(
                `${process.env.NEXT_PUBLIC_STRAPI_URL}/reviews`,
                {
                    ...
                    body: JSON.stringify({ data: { ...form, professional: id, client_profile: profileId } }),
                    method: 'POST',
                }
            );
            ...
            if (responseData?.data) {
                ...
                setReviewsLoader(!reviewsLoader)
                setForm({
                    // email: '',
                    // title: '',
                    rating: '',
        ...
        <section className='bg-[#FCFCFC] mt-8 flex flex-col space-y-3 rounded-[10px] w-full  p-[20px]'>
            ...
            <form className='w-full my-3' onSubmit={(e) => handleSubmit(e)}>
                    ...
                    <div className='w-full md:w-[50%] flex flex-col space-y-2'>
                        ...
                        >
                            <SelectTrigger className=" bg-white text-[14px] w-full text-[#1A1A1A] leading-[18px] font-normal">
                                <SelectValue placeholder="4 Star - Good" />
                            </SelectTrigger>
                            <SelectContent >
                ...
                <div className='w-full flex flex-col space-y-2 my-4'>
                    ...
                    <Textarea required minLength={7} className='' placeholder='write a review' name='review' value={form.review} onChange={handleChange} />
                </div>
